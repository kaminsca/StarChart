...
def parse_bright_star_catalog(
    ...
    for _, row in df.iterrows():
        ...
        ra_m = row["RAm"]
        ra_s = row["RAs"]
        ra_deg = (ra_h + ra_m / 60 + ra_s / 3600) * 15  # 1h = 15deg

        # Declination J2000 to decimal degrees
        dec_sign = 1 if str(row["DE-"]).strip() != "-" else -1
        dec_d = row["DEd"]
        dec_m = row["DEm"]
        dec_s = row["DEs"]
...
# https://www.kaggle.com/datasets/anavid7/yale-bright-star-catalog?resource=download&select=yale_bright_star_catalog_v5.csv
